<template>
  <v-card class="pa-5">
    <v-card-title class="headline primary--text">Add a beam</v-card-title>
        ...
        <v-container fluid class="px-0">
          <v-row class="ma-0 px-0">
            <v-col order="1" cols="12" md="4">
              <v-text-field
                label="Name*"
                required
                hint="Name of the beam. This field cannot be changed later."
                persistent-hint
                v-model="form.name"
                :rules="nameRules"
              ></v-text-field>
              ...
              >
                <template v-slot:activator="{ on }">
                  <v-text-field
                    ...
                    required
                    hint="The date on which the beam was produced, e.g., 2020-05-06. This field cannot be changed later."
                    persistent-hint
            ...
            <v-col order="4" cols="6" md="4">
              <v-text-field
                label="Produced by*"
                required
                hint="The person or group that produced the beam, e.g. pwg-xxx. This field cannot be changed later."
                persistent-hint
            ...
            <v-col order="4" cols="6" offset-md="4" md="4">
              <v-text-field
                label="Contact*"
                required
                hint="A person or group that can be contacted for questions or issues about the beam."
                persistent-hint
          ...
          <v-row class="mx-0 mb-3 px-0">
            <v-col cols="12" md="8" offset-md="4">
              <v-textarea
                label="Paths"
                hint="A path per line. e.g., nersc:/go/to/my/beams_v3"
                rows="2"
                persistent-hint
                v-model="form.paths"
              ></v-textarea>
...
import CREATE_BEAM from "@/graphql/CreateBeam.gql";
    ...
    async addProduct() {
      try {
        ...
        const data = await this.$apollo.mutate({
          mutation: CREATE_BEAM,
          variables: { input: createProductInput },
          update: (cache, { data: { createBeam } }) => {
            const data = cache.readQuery({
              query: ALL_BEAMS
            });
            data.allBeams.edges.splice(0, 0, {
              node: createBeam.beam,
              __typename: "BeamEdge"
            });
            cache.writeQuery({
              query: ALL_BEAMS,
